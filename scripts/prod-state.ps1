...
select 'server=' || coalesce(host(inet_server_addr()),'SOCKET-LOCAL')
    ...
    || ' | ui_fk=' || (select count(*)::text from pg_constraint c join pg_class t on t.oid = c.conrelid where c.contype = 'f' and t.relname = 'user_import_batches' and c.confrelid = to_regclass('public.profiles'))
