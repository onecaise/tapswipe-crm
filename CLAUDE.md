...
`audit_log` has no insert policy either, which is what decides where an audited action has to live. Anything that must leave a trail runs as service role (an Edge Function) or as a `security definer` RPC — a plain client write cannot log itself. `set_user_role()` is `security definer` for that reason alone, since `profiles` already has an admin UPDATE policy that would otherwise have been enough. `authenticated` holds **`select` only** on `audit_log`; the security audit narrowed it, because write privileges there were previously held back by nothing but the absence of a policy for those verbs.
